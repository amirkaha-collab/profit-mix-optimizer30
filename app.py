...
def _render_compare_cards(df_sel: pd.DataFrame, title: str = "השוואה"):
    ...
    st.markdown(f"#### {_escape_html(title)}", unsafe_allow_html=True)

    # Responsive: cards stack on mobile, no horizontal scroll
    st.markdown(
        """
        <style>
        .cmp-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(290px,1fr));gap:14px;margin-top:10px;}
        .cmp-card{background:#ffffff;border:1px solid rgba(148,163,184,.35);border-radius:18px;box-shadow:0 10px 26px rgba(2,6,23,.08);padding:14px 14px 10px 14px;}
        .cmp-head{display:flex;flex-direction:column;gap:4px;margin-bottom:10px;}
        .cmp-fund{font-weight:850;font-size:16px;line-height:1.25;color:#0b1220;letter-spacing:.2px;}
        .cmp-sub{font-size:12px;color:#475569;}
        .cmp-kpis{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin:10px 0 10px 0;}
        .cmp-kpi{background:linear-gradient(180deg,#fbfdff,#f6f8fb);border:1px solid rgba(148,163,184,.35);border-radius:14px;padding:10px;}
        .cmp-kpi .k{font-size:11px;color:#64748b;margin-bottom:3px;font-weight:700;}
        .cmp-kpi .v{font-weight:900;color:#0b1220;font-size:14px;}
        .cmp-mini{width:100%;border-collapse:separate;border-spacing:0 7px;margin-top:6px;}
        .cmp-mini td{padding:9px 10px;background:linear-gradient(180deg,#fbfdff,#f6f8fb);border:1px solid rgba(148,163,184,.35);}
        .cmp-mini td:first-child{border-radius:12px 0 0 12px;color:#475569;font-size:12px;font-weight:800;width:46%;}
        .cmp-mini td:last-child{border-radius:0 12px 12px 0;color:#0b1220;font-size:13px;font-weight:900;text-align:left;}
        .rtl .cmp-mini td:last-child{text-align:right;}
        </style>
        """,
        unsafe_allow_html=True,
    )
